  ...
  return (
    ...
    >
      <div 
        ref={modalRef}
        tabIndex={-1}
        className="bg-background-surface border border-border-dark w-full h-full lg:h-[90vh] lg:max-w-[1200px] lg:rounded-2xl rounded-none flex flex-col shadow-2xl overflow-hidden outline-none" 
        onClick={(e) => e.stopPropagation()}
      >
        
        {/* Header */}
        <header className="p-3 md:p-6 border-b border-border-dark flex justify-between items-start bg-background-surface shrink-0">
          <div className="flex gap-4">
            ...
            <div>
              <h2 id="modal-title" className="text-text-primary text-lg font-bold leading-tight line-clamp-1">{localTicket.subject}</h2>
          ...
          <div className="flex items-center gap-3">
             ...
             {(hasPrev !== undefined || hasNext !== undefined) && (
                 <div className="hidden md:flex items-center bg-background-card rounded-lg border border-border-dark mr-2">
                     <button 
                        onClick={onPrev} 
                        disabled={!hasPrev}
                        className="p-1.5 hover:bg-background-input disabled:opacity-30 disabled:hover:bg-transparent transition-colors rounded-l-lg border-r border-border-dark flex items-center justify-center"
                     ...
                     <button 
                        onClick={onNext} 
                        disabled={!hasNext}
                        className="p-1.5 hover:bg-background-input disabled:opacity-30 disabled:hover:bg-transparent transition-colors rounded-r-lg flex items-center justify-center"
        ...
        <div className="flex-1 overflow-hidden grid grid-cols-1 lg:grid-cols-[1fr_320px]">
          ...
          <div className={`flex flex-col h-full bg-background-surface border-r border-border-dark min-h-0 ${activeTab === 'details' ? 'hidden lg:flex' : 'flex'}`}>
            
            {/* Tabs */}
            <div className="flex items-center gap-6 px-6 border-b border-border-dark shrink-0 bg-background-surface">
              <button
            ...
            {activeTab === 'messages' ? (
                <>
                        ...
                        <div className="flex flex-col gap-5">
                            {localTicket.messages && localTicket.messages.map((msg, i) => {
                                ...
                                return (
                                    ...
                                    >
                                      <div
                                        className="size-8 rounded-full overflow-hidden shrink-0 bg-background-card border border-border-dark flex items-center justify-center text-[10px] font-bold text-text-secondary"
                                        style={
                                          msg.senderAvatar
                                      ...
                                      >
                                        <div className="flex items-center gap-2">
                                          <span className="text-text-primary text-[11px] font-bold">{msg.senderName}</span>
                                          {isInternal && (
                                        ...
                                        <div
                                          className={`p-3 rounded-xl text-sm leading-relaxed break-words ${
                                            isMe
                                              ? isInternal
                                                ? 'bg-yellow-500/20 text-text-primary border border-yellow-500/40 rounded-tr-none'
                                                : 'bg-primary text-white rounded-tr-none'
                                              : isInternal
                                                ? 'bg-background-card text-yellow-50 border border-yellow-500/40 rounded-tl-none'
                                                : 'bg-background-card text-text-primary border border-border-dark rounded-tl-none'
                                          }`}
                                        >
                                          {msg.content}
                                        </div>
                    ...
                    {/* Input Area */}
                    <div className="p-3 md:p-6 border-t border-border-dark bg-background-surface">
                        {isAssignedToOthers && user?.profile !== 'Cliente' && !isCreator ? (
                            <div className="flex items-center justify-center p-3 md:p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl text-yellow-500 text-sm gap-2">
                                ...
                                <button onClick={handleTakeTicket} className="ml-2 underline hover:text-text-primary">Assumir Chamado</button>
                            </div>
                            ...
                            <>
                                <div className="relative mb-4 group">
                                    {selectedAttachment && (
                                        <div className="absolute bottom-full left-0 mb-2 bg-background-card border border-border-dark rounded-lg p-2 flex items-center gap-2 text-xs text-text-primary animate-fade-in">
                                    ...
                                    className="w-full h-[80px] md:h-[120px] bg-background-input border border-border-dark rounded-xl p-3 md:p-4 text-sm text-text-primary focus:ring-1 focus:ring-primary outline-none resize-none transition-all placeholder:text-text-muted"
                                    value={replyText}
                                ...
                                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                                        ...
                                        <button
                                          ...
                                          className={`inline-flex items-center justify-center px-3 py-2 rounded-lg text-[11px] font-bold border transition-colors ${
                                            isInternalNote
                                              ...
                                              : 'bg-background-surface border-border-dark text-text-secondary hover:text-text-primary'
                                          }`}
                                        >
                                    ...
                                    />
                                        <button 
                                            ...
                                            className={`px-3 md:px-5 h-10 border border-border-dark text-text-primary text-xs font-bold rounded-lg hover:bg-background-card transition-all flex items-center gap-2 ${selectedAttachment ? 'bg-primary/20 border-primary/50' : ''}`}
                                        >
